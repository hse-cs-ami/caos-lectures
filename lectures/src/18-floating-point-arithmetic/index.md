...
Обозначим знаковый бит как $sign$; беззнаковое значение показателя степени как $exp$ и беззнаковое значение дробной части мантиссы как $frac$, а их битовые представления с нижним индексом $_2$.
...
Бесконечности нужны для обозначения очень больших по модулю чисел, для представления которых не достаточно восьми бит показателя степени.  
...
* $f = -(-f)$ $-$ верно всегда.
* $(d < 0) \Rightarrow (2d < 0)$ $-$ верно всегда, в том числе если получается -inf.
* $(d > f) \Rightarrow (-f < -d)$ $-$ верно всегда
* $d \cdot d \geq 0$ $-$ верно всегда
...
Мы не будем передавать и возвращать float-ы и double-ы из функций, потому что это использует устаревший набор инструкций сопроцессора. Поэтому мы будем работать с указателями на массивы вещественных чисел. Все регистры XMM caller-saved (не надо сохранять внутри функции).
...
Чтобы печатать float-ы через printf на 32-битной архитектуре, используются инструкции x87. А на 64-битной параметры типа float и double передаются через регистры xmm. Подробнее не будем изучать.
...
Идея в том, что отдельно неточно считается сумма больших по модулю чисел (переменная `sum`). И отдельно считается компенсация для накопленной ошибки округления (переменная `c`). 
